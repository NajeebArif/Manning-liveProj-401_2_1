...
public interface UsersApi {
    ...
    @Operation(summary = "List all Users", description = "", security = {
        @SecurityRequirement(name = "basicAuth")    }, tags={ "users" })
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "A paged array of users", content = @Content(schema = @Schema(implementation = Users.class))),
    ...
    ResponseEntity<Users> listUsers(@Parameter(in = ParameterIn.QUERY, description = "How many items to return at one time (max 100)" ,schema=@Schema()) @Valid @RequestParam(value = "limit", required = false) Optional<Integer> limit,
                                    @Parameter(in = ParameterIn.QUERY, description = "What is the current pagination page" ,schema=@Schema()) @Valid @RequestParam(value = "page", required = false) Optional<Integer> page);


    @Operation(summary = "Info for a specific user", description = "", security = {
        @SecurityRequirement(name = "basicAuth")    }, tags={ "users" })
    @ApiResponses(value = { 
        @ApiResponse(responseCode = "200", description = "Expected response to a valid request", content = @Content(schema = @Schema(implementation = User.class))),
